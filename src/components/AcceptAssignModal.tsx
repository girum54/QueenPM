        ...
        <div className="px-5 py-4 border-b border-slate-800 flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[11px] uppercase tracking-wider font-semibold text-fuchsia-300">
              {isStakeholder ? "Task Details (View Only)" : "Accept & Assign"}
            </div>
        ...
        <div className="p-5 space-y-5">
          ...
          >
            <select
              ...
              className="w-full h-9 rounded-md bg-slate-800/60 border border-slate-700 px-3 text-sm text-slate-100 outline-none focus:border-fuchsia-500 disabled:opacity-80 disabled:cursor-not-allowed"
            >
          ...
          >
            <div className="flex gap-1 p-1 rounded-md bg-slate-800/60 border border-slate-700 mb-2 w-fit">
              <button
                type="button"
                onClick={() => !isStakeholder && setMode("deadline")}
                disabled={isStakeholder}
              ...
              <button
                type="button"
                onClick={() => !isStakeholder && setMode("days")}
                disabled={isStakeholder}
                className={`px-3 h-7 rounded text-xs font-medium inline-flex items-center gap-1.5 transition ${
            ...
            {mode === "deadline" ? (
              <div className="space-y-2">
                <DatePicker
                  ...
                  disabled={isStakeholder}
                />
                {sprintEndDate && deadline !== sprintEndDate && !isStakeholder && (
                  <button
                    type="button"
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  disabled={isStakeholder}
                  className="w-24 h-9 rounded-md bg-slate-800/60 border border-slate-700 px-3 text-sm text-slate-100 outline-none focus:border-fuchsia-500 disabled:opacity-80 disabled:cursor-not-allowed"
                />
          ...
          <Field label="Route To Column" hint={isStakeholder ? undefined : "Move this card straight out of New"}>
              ...
              {COLUMNS.map((c) => {
                ...
                return (
                  <button
                    key={c}
                    type="button"
                    onClick={() => !isStakeholder && setColumn(c)}
                    disabled={isStakeholder}
          ...
          <Field label="Priority Confirmation" hint={isStakeholder ? undefined : "Override the suggested severity"}>
            <div className="grid grid-cols-4 gap-1.5">
              {(["low", "medium", "high", "urgent"] as Priority[]).map((p) => {
                ...
                return (
                  <button
                    key={p}
                    type="button"
                    onClick={() => !isStakeholder && setPriority(p)}
                    disabled={isStakeholder}
        ...
        <div className="px-5 py-3.5 border-t border-slate-800 flex items-center justify-end gap-2">
          {isStakeholder ? (
            <button
              type="button"
              onClick={onClose}
              className="h-9 px-5 rounded-md text-xs font-semibold bg-slate-800 hover:bg-slate-700 text-slate-200 transition"
            >
              Close
            </button>
          ) : (
